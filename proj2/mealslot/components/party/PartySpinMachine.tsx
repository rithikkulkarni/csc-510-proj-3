 ...
 * PartySpinMachine
 * ------------------------------------------------------------
 * UI component for the multiplayer (party) slot machine.
 * Handles rendering of slot reels, host-only controls (spin/lock),
 * per-slot voting, power-ups, and recent spin history.
 */
 ...
 * Pill
 * ---------------------------------------------------
 * Generic pill-style label (currently unused here, but kept
 * as a shared UI primitive for consistency).
 */
 ...
 * ToggleChip
 * ---------------------------------------------------
 * Reusable pill-shaped toggle used for power-up controls.
 * Visual state switches based on `active`.
 */
function ToggleChip({
  active,
  children,
 ...
 * Ribbon
 * ---------------------------------------------------
 * Small section title used at the top of each card.
 */
 ...
 * Card
 * ---------------------------------------------------
 * Shared card container used to visually group sections
 * (power-ups, recent spins, etc.) in the party UI.
 */
 ...
 * PartySpinMachine
 * ---------------------------------------------------
 * Core “party mode” slot machine UI:
 * - Renders three reels with the current party-wide dish selection.
 * - Allows host-only locking of individual slots.
 * - Lets each member vote per slot (“keep” or “reroll”).
 * - Provides power-up toggles and host controls for group spin / reroll.
 * - Mirrors the solo slot machine styling while layering in multiplayer state.
 */
export default function PartySpinMachine({
   ...
   * -------------------------------------------------
   * Single-slot card:
   * - Shows dish name + category (or “No selection”).
   * - Host-only lock/unlock button for the slot.
   * - Member voting buttons for keep / reroll.
   * - Quick link to YouTube search for the dish recipe.
   */
    ...
    return (
      <div className="relative flex flex-col justify-between rounded-2xl border border-neutral-200 bg-white/90 p-3 shadow-sm transition-all duration-300 ease-out hover:-translate-y-0.5 hover:border-orange-300 hover:shadow-md min-h-[200px]">
        ...
        <div className="space-y-2">
          {/* Host-only lock/unlock for this slot */}
          <button
            type="button"
          ...
          >
            {locks[idx] ? (
              <>
                <Unlock className="h-4 w-4 inline mr-1" /> Unlock
              </>
            ) : (
              <>
                <Lock className="h-4 w-4 inline mr-1" /> Lock
              </>
            )}
          </button>

          {/* Per-slot votes + recipe link */}
          {slot && (
            <>
              <div className="flex gap-2">
                ...
                >
                  <RotateCcwIcon className="h-3 w-3 inline mr-1" />{" "}
                  {v.reroll.size}
                </button>
              </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Slot Machine Frame - visually aligned with solo slot machine */}
        ...
        <div className="mb-6 rounded-2xl border-2 border-[#2B6D82] bg-[#123040] p-6 shadow-inner">
          <div className="grid gap-4 md:grid-cols-3">
            {[0, 1, 2].map((idx) => {
              const slotCat =
                slotCategories?.[idx] ||
                ["Breakfast", "Lunch", "Dinner"][idx];
              return (
                <div
                  key={idx}
                  className="flex flex-col items-center gap-2"
                >
                  {/* Per-slot category selector (optional, host-configured from parent) */}
                  {onCategoryChange && (
                    <select
                      value={slotCat}
                      onChange={(e) => onCategoryChange(idx, e.target.value)}
                      disabled={isSpinning}
                      className="w-full rounded-lg border border-[#2B6D82] bg-[#0F1C24] px-3 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-[#1F4F61] focus:outline-none focus:ring-2focus:ring-[#F1C04F] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
