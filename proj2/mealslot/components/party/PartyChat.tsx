 ...
 * Ribbon
 * ---------------------------------------------------
 * Small title helper used at the top of the chat card.
 */
 ...
 * Card
 * ---------------------------------------------------
 * Simple shared “card-style” container used to wrap the chat UI.
 */
 ...
 * PartyChat
 * ---------------------------------------------------
 * Lightweight party chat panel:
 * - Displays a scrollable list of messages with sender + timestamp.
 * - Provides a text input and send button.
 * - Delegates sending behavior to the parent via `onSendChat`.
 */
export default function PartyChat({ chat, onSendChat }: PartyChatsProps) {
  const chatInputRef = useRef<HTMLInputElement>(null);

  /**
   * Read the current input value, clear the field, and forward
   * the text to the parent handler.
   */
  const handleSend = () => {
        ...
        {chat.length === 0 ? (
          ...
          chat.map((m) => (
            <div key={m.id} className="mb-1">
              <span className="rounded bg-sky-600/20 px-1 py-0.5 font-medium">
                {m.from}
              </span>{" "}
              <span className="opacity-60">
                {new Date(m.ts).toLocaleTimeString()}
              </span>
              <div className="pl-1 text-brand-dusk dark:text-white">
                {m.text}
              </div>
            </div>
          ))
